...
```mermaid
flowchart LR
    A[요구사항] -->|❌ 내부 plan만| B1[즉시 코드 작성]
    B1 --> C1[방향 틀림을 결과 봐야 발견]
    C1 --> D1[롤백 + 재시도]

    A -->|✅ 외부 plan| B2[plan을 화면에 노출]
    B2 --> V{사람 검토}
    V -->|수정 요청| B2
    V -->|승인| E[코드 작성]
    E --> F[방향 일치]
...
모든 작업에 Plan Mode를 거는 건 과합니다. 학습자가 자주 던지는 질문 — "어떤 작업에 켜야 하나요?" 에 대한 결정 가이드입니다.

| 신호 | 권장 | 이유 |
|---|---|---|
| 변경 범위 추정이 어려움 | ✅ 켠다 | 범위 외부화가 가장 큰 가치 |
| PR로 올릴 작업 | ✅ 켠다 | 리뷰어와 plan을 공유 가능 |
| 익숙하지 않은 모듈을 건드림 | ✅ 켠다 | AI 추측 위험이 큼 |
| Production 코드 변경 | ✅ 켠다 | 사후 발견 비용이 큼 |
| 단일 파일 한두 줄 수정 | 생략 | plan 작성 시간이 절약 가치 초과 |
| 탐색·프로토타입·toy 작업 | 생략 | 빨리 틀리고 빨리 배우는 게 이득 |

판단 기준 한 줄: **"이 변경이 잘못 가면, 코드를 보고 발견하면 늦는가?"** Yes면 Plan Mode.
...
<Tabs items={['Claude Code', 'Codex', 'AI Pro']}>
  ...
  <Tabs.Tab>
    Codex는 별도 Plan Mode 단축키 대신 **Approval Policy(`suggest` / `auto-edit` / `full-auto`)** 로 동등한 효과를 만듭니다. 가장 안전한 흐름은 **명시적 "Explore → Plan → Code" 프롬프트 패턴**입니다.

    ```bash
    codex
    # 첫 메시지: 계획만 먼저
    "변경하지 말고 계획만 먼저 보여줘. 어떤 파일을 어떤 순서로 수정할지,
    그리고 완료를 어떻게 검증할지 포함해서."
    ```

    Plan을 검토·수정 후 다음 메시지로:
    ```
    이 계획대로 진행해줘. 단, models.py는 건드리지 마.
    ```
  </Tabs.Tab>
  <Tabs.Tab>
...
같은 요구사항이지만, 이번엔 도구별로 **외부 plan**을 먼저 받아냅니다.

<Tabs items={['Claude Code', 'Codex', 'AI Pro']}>
  <Tabs.Tab>
    `Shift + Tab`으로 Plan Mode 진입 후 동일한 요구사항 입력. Plan만 출력되고 코드는 변경되지 않습니다.
  </Tabs.Tab>
  <Tabs.Tab>
    명시적 프롬프트로 plan을 외부화합니다:
    ```
    프로젝트 생성 시 입력값 유효성 검증을 추가하려고 해.
    변경하지 말고 계획만 먼저 보여줘:
    - 어떤 파일을 어떤 순서로 수정할지
    - 완료를 어떻게 검증할지
    ```
    plan을 검토·수정한 후 다음 메시지로 "이 계획대로 진행해줘"라고 승인합니다.
  </Tabs.Tab>
  <Tabs.Tab>
    AI Pro의 계획 검토 기능 사용 또는, 명시적 프롬프트:
    ```
    프로젝트 생성 시 입력값 유효성 검증을 추가하려고 해.
    먼저 계획만 보여줘 — 파일·순서·검증 방법 포함해서. 코드는 쓰지 마.
    ```
    Plan을 메모 또는 AGENTS.md 옆에 보관 후, 동일한 Chat에서 "이 계획대로 진행"으로 승인합니다.
  </Tabs.Tab>
</Tabs>
