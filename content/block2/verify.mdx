...
검증 루프가 없으면:
- AI가 "완료했습니다"라고 해도 실제로 동작하는지 모릅니다
- 에러가 숨겨진 채로 코드가 쌓인다
- 사람이 매번 수동으로 확인해야 합니다

검증 루프가 있으면:
- AI가 테스트를 실행하고 실패하면 **스스로 수정**합니다
- 사람은 최종 결과만 검토하면 됩니다
- 품질이 2~3배 올라간다
...
- 작업 완료 후 반드시 모든 테스트가 통과해야 합니다
- 새 기능을 추가할 때 테스트를 함께 작성합니다
```

이 항목이 있으면 Claude가 작업 후 **스스로 테스트를 실행**합니다.
...
<Steps>
### Step 1 — 컨텍스트 파일에 검증 방법 추가 (5분)

위 내용을 도구의 컨텍스트 파일에 추가합니다.

<Tabs items={['Claude Code', 'Codex', 'AI Pro']}>
  <Tabs.Tab>
    `sds-harness-lab/CLAUDE.md` 에 추가합니다.
  </Tabs.Tab>
  <Tabs.Tab>
    `sds-harness-lab/AGENTS.md` 에 추가합니다.
  </Tabs.Tab>
  <Tabs.Tab>
    `sds-harness-lab/AGENTS.md` 또는 Rules 파일에 추가합니다.
  </Tabs.Tab>
</Tabs>

### Step 2 — 검증 루프 연결 실습 (20분)

도구를 열고 동일한 프롬프트를 입력합니다. 핵심은 **AI가 자기 결과를 직접 검증하게 만드는 한 문장**입니다.
...
<Tabs items={['Claude Code', 'Codex', 'AI Pro']}>
  <Tabs.Tab>
    Claude Code는 `Bash` 도구로 `pytest`를 직접 실행합니다 — 별도 승인 없이 검증 루프가 돌아갑니다.
  </Tabs.Tab>
  <Tabs.Tab>
    Codex는 approval policy에 따라 명령 실행 전 승인을 묻습니다. 검증 루프 실습에서는 `auto-edit` 또는 `full-auto` 모드로 두면 Claude Code와 같은 흐름이 됩니다 (단, 코드 변경도 자동 적용되므로 신뢰할 수 있는 코드베이스에서만).
  </Tabs.Tab>
  <Tabs.Tab>
    AI Pro에서 터미널 실행을 허용한 후 동일 프롬프트를 입력합니다. 도구가 `pytest`를 실행할 수 없는 환경이면, 사람이 직접 `pytest`를 돌려 결과를 다음 메시지에 붙여넣어 줍니다 — 이게 수동 검증 루프.
  </Tabs.Tab>
</Tabs>
...
1. name이 빈 문자열이면 ValueError가 발생해야 합니다
2. status가 허용 목록에 없으면 ValueError가 발생해야 합니다
