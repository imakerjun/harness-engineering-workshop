...
<Tabs items={['Claude Code', 'Codex', 'AI Pro']}>
  ...
  <Tabs.Tab>
    ...
    /compact
    ```
    Codex도 동일하게 슬래시 명령으로 대화를 압축합니다. 명령이 안 보이면 `/help`로 현재 빌드의 단축어를 확인하세요.

    **언제**: Claude Code와 같음 — 한 작업 단락이 끝났을 때
  </Tabs.Tab>
  <Tabs.Tab>
...
<Tabs items={['Claude Code', 'Codex', 'AI Pro']}>
  ...
  <Tabs.Tab>
    `/compact`로 압축하거나, 세션을 종료한 뒤 AGENTS.md에 "지금까지 결정한 것"을 추가하고 새 `codex` 세션을 시작합니다. 두 방식의 결과 품질을 비교해보세요.
  </Tabs.Tab>
  <Tabs.Tab>
...
```markdown
## 🗂️ 세션 관리
- 작업이 5턴 이상 이어지면 /compact 후 새 세션으로 분리합니다
- 세션 간 이어지는 결정은 이 파일의 "최근 결정" 섹션에 기록합니다
```
</Steps>
